...
impl EnvironmentBuilder {
    ...
    pub fn open_with_permissions(&self, path: &Path, mode: ffi::mode_t) -> Result<Environment> {
        ...
        unsafe {
            ...
            if let Some(page_size) = self.page_size {
                lmdb_try_with_cleanup!(ffi::mdb_env_set_pagesize(env, page_size),
                                       ffi::mdb_env_close(env))
            }
    ...
    /// Sets the size of database pages for the environment.
    ///
    /// The size must be a power of two. It applies to environments whose data
    /// file does not yet exist; an existing file keeps the page size it was
    /// created with.
    pub fn set_page_size(&mut self, page_size: c_int) -> &mut EnvironmentBuilder {
